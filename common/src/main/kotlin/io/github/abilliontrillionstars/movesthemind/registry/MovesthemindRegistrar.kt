package io.github.abilliontrillionstars.movesthemind.registry
...
typealias RegistrarEntry<T> = MovesthemindRegistrar<T>.Entry<out T>

abstract class MovesthemindRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Movesthemind.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is MovesthemindRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
