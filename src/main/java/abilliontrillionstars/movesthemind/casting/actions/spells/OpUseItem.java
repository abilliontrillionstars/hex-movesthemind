...
public class OpUseItem implements SpellAction
{
    public static final Action INSTANCE = new OpUseItem();
    ...
    public @NotNull SpellAction.Result executeWithUserdata(@NotNull List<? extends Iota> args, @NotNull CastingEnvironment env, @NotNull CompoundTag tags)
    {
        ...
        return new SpellAction.Result(new OpUseItem.Spell(target, frequency),
    ...
    private class Spell implements RenderedSpell
    {
        ...
        public void cast(@NotNull CastingEnvironment env)
        {
            ...
            {
                case 0:
                    server.getCommands().performPrefixedCommand(sourceStack, "player " + username + " use continuous");
                    break;
                case -1:
                    server.getCommands().performPrefixedCommand(sourceStack, "player " + username + " use once");
                    break;
                default:
                    server.getCommands().performPrefixedCommand(sourceStack, "player " + username + " use interval "+frequency);
                    break;
            }
        }
